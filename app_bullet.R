  ...
  observeEvent({length(c(input$gemeinde_a,input$gemeinde_b))==2},{
    ...
    if (length(selected_gemeinden)==2 ){
      if (input$gemeinde_a != input$gemeinde_b){
        ...
        output$pstk_text <- renderUI({
          generate_bullets_pstk(echart_pstk_data, selected_gemeinden)
        ...
        output$winlose_text <- renderUI({
          generate_bullets_winlose(winlose_data = win_lose,selected_gemeinden = selected_gemeinden)
        })
        ...
        output$panasch_text_a <- renderUI({
          generate_bullets_panasch(sankey_panasch_data = echart_sankey_data_a,attrakt_data = attrakt_a,gemeinde = input$gemeinde_a)
        })

        output$panasch_text_b <- renderUI({
          generate_bullets_panasch(sankey_panasch_data = echart_sankey_data_b,attrakt_data = attrakt_b,gemeinde = input$gemeinde_b)
        })
